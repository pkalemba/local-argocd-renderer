module github.com/pkalemba/local-argocd-renderer
...
// Argo CD is consumed as a library here, but its own go.mod resolves several
// dependencies through replace directives, which do not apply downstream:
// gitops-engine lives in the Argo CD repo and is pinned to a pseudo-version that
// only resolves through `replace ... => ./gitops-engine`, and the k8s.io staging
// modules are pinned to v0.0.0. The block below mirrors the pins from Argo CD's
// go.mod, with gitops-engine pointed at the published module version built from
// the same commit as the argo-cd tag above.
replace (
